// ============================================================
// Battery Voltage Monitor v2.0 — All-in-one
...
#define DEVICE_ID    "battery_monitor"
...
void connectWiFi() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.setHostname("battery-monitor");
...
void publishDiscovery() {
  ...
  // --- Voltage ---
  {
    String p = "{\"name\":\"Battery Voltage\",\"unique_id\":\"bat_mon_v\",";
    p += state + "," + avail + ",";
  ...
  // --- SOC ---
  {
    String p = "{\"name\":\"Battery SOC\",\"unique_id\":\"bat_mon_soc\",";
    p += state + "," + avail + ",";
  ...
  // --- Voltage Trend ---
  {
    String p = "{\"name\":\"Voltage Trend\",\"unique_id\":\"bat_mon_trend\",";
    p += state + "," + avail + ",";
  ...
  // --- Trend Label ---
  {
    String p = "{\"name\":\"Battery Trend\",\"unique_id\":\"bat_mon_label\",";
    p += state + "," + avail + ",";
  ...
  // --- Chemistry Select ---
  {
    String p = "{\"name\":\"Battery Chemistry\",\"unique_id\":\"bat_mon_chem\",";
...
void connectMQTT() {
  ...
  String clientId = "bat_mon_" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void setup() {
  ...
  Serial.println();
  Serial.println("===================================");
  Serial.println("  Battery Voltage Monitor v2.0");
  ...
  Serial.printf("[READY] OTA: battery-monitor | IP: %s\n",
                WiFi.localIP().toString().c_str());
}
